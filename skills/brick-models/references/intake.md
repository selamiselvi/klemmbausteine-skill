...
All canonical copy and helper output are English. The agent presents them in the user's language, including descriptions and mode choices, and maps the answers back to the unchanged option IDs. Translate wording, not the planning rules or measurements. No language-specific source copies are needed.
...
For example, “Just make a Japanese temple” could warrant a medium model with balanced detail to leave room for a recognizable layered roof, columns and entrance. This is a design judgment, not a mandatory temple preset. A request for a tiny temple retains the small scale and still calls for the best-fitting parts and construction techniques. Auto mode does not imply a preference for stylization; follow the fidelity and engine-limit guidance in `SKILL.md` without asking routine design questions.
...
All profiles seek the closest likeness within the user's constraints. Detail presets and estimated parts ranges are planning aids, not permission to leave achievable fidelity unused; only an explicit preference for simplification, limited parts or complexity narrows that objective. A detailed profile does not itself enable unsupported geometry. Follow `SKILL.md` when required parts or techniques exceed the current engine; do not treat automatic planning as acceptance of a less faithful substitute.
